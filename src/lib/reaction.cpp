...
{
  if (_emu_prime_mag2 != _emu_prime_mag2)
    SetElec();
...
void Reaction::SetPip(int i)
{
  ...
  _dc_Region1_edge = _data->dc_r1_edge(i);
  _dc_Region2_edge = _data->dc_r2_edge(i);
  _dc_Region3_edge = _data->dc_r3_edge(i);
}
...
{
  if (_excl_Mom != _excl_Mom)
    CalcMissMass();
  return _excl_Mom;
  // else
  // return NAN;
}
float Reaction::Pip_theta_mes()
{
  if (TwoPion_exclusive())
    return _pip->Theta() * 180 / PI;
  else
    return NAN;
}

float Reaction::Pip_phi_mes()
{
  if (TwoPion_exclusive())
  {
    // if (_pip->Phi() >= 0)
...
// // Function to rotate a Point3D around Z and Y
// TVector3 Reaction::trajTiltedPoint3D(TVector3 &Point3D, int sec)
// {
//   Point3D.RotateZ(-TMath::Pi() / 3.0 * (sec - 1));
//   Point3D.RotateY(TMath::Pi() / 180.0 * (-25));
//   return Point3D;
// }
// TVector3 Reaction::trajLocalPoint3D(TVector3 &Point3D, int sec, int layer, int detID)
// {
//   Point3D.rotateZ(TMath::Pi() / 2 - TMath::Pi() / 3 * (sec - 1));
//   if (layer == 3 && detector == DetectorType.FTOF.getDetectorId())
//     Point3D.rotateX(TMath::Pi() / 180.0 * (58.11));
//   else
//     Point3D.rotateX(TMath::Pi() / 180.0 * (25));
//   return Point3D;
// }

// void Reaction::Calc_theta_phi(int i)
// {
//   int sec = _data->dc_sec(i);
//   // int sec = _data->dc_sec(0);

//   // if (sec == 1) {
//   Double_t x = _data->dc_r1_x(i);
//   Double_t y = _data->dc_r1_y(i);
//   Double_t z = _data->dc_r1_z(i);

//   TVector3 xyz(_data->dc_r1_x(i), _data->dc_r1_y(i), _data->dc_r1_z(i));
//   TVector3 trajTiltedPoint3D_r1 = trajTiltedPoint3D(xyz, sec);
//   // TVector3 residual = xyz;
//   _theta_PIP_traj_tilt = trajTiltedPoint3D_r1.Theta() * 180 / PI;
//   _phi_PIP_traj_tilt = trajTiltedPoint3D_r1.Phi() * 180 / PI;
// }

// float Reaction::Pip_theta_traj_tilt()
// {
//   return _theta_PIP_traj_tilt;
// }

// float Reaction::Pip_theta_traj_tilt()
// {
//   return _phi_PIP_traj_tilt;
// }
