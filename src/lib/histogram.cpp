...
Histogram::Histogram(const std::string &output_file)
{
  ...
  W_vs_Q2 = std::make_shared<TH2D>("W_vs_Q2", "W_vs_Q2", bins, w_min, w_max, bins, zero, q2_max);

  W_mc_hist = std::make_shared<TH1D>("W_mc", "W_mc", bins, w_min, w_max);
  Q2_mc_hist = std::make_shared<TH1D>("Q2_mc", "Q2_mc", bins, zero, q2_max);
  W_vs_Q2_mc = std::make_shared<TH2D>("W_vs_Q2_mc", "W_vs_Q2_mc", bins, w_min, w_max, bins, zero, q2_max);
...
void Histogram::Fill_WvsQ2_mc(const std::shared_ptr<MCReaction> &_e_mc)
{
  short sec = _e_mc->sec();

  W_mc_hist->Fill(_e_mc->W_mc(), _e_mc->weight());
  Q2_mc_hist->Fill(_e_mc->Q2_mc(), _e_mc->weight());
  W_vs_Q2_mc->Fill(_e_mc->W_mc(), _e_mc->Q2_mc(), _e_mc->weight());
}
...
void Histogram::Write_WvsQ2()
{
  ...
  if (W_vs_Q2_mc->GetEntries())
    W_vs_Q2_mc->Write();

  W_mc_hist->SetXTitle("W (GeV)");
  if (W_mc_hist->GetEntries())
    W_mc_hist->Write();

  Q2_mc_hist->SetXTitle("Q^{2} (GeV^{2})");
  if (Q2_mc_hist->GetEntries())
    Q2_mc_hist->Write();
}
...
void Histogram::Fill_deltat_prot(const std::shared_ptr<Branches12> &data, const std::shared_ptr<Delta_T> &dt, int part,
                                 const std::shared_ptr<Reaction> &_e)
{
  auto _cuts = std::make_unique<Cuts>(data, dt);
  ...
  if (cd_part)
  {
    ...
    time = dt->dt_ctof_P();
    // std::cout << "  ctof proton  ............: " << data->status(part) << std::endl;
  }
  else if (fd_part)
  {
    // std::cout << "ftof : " << data->status(part) << std::endl;
    time1 = dt->dt_P();
  }
  if (charge == 1)
  {
    ...
    if (_cuts->IsProton(part))
    {
      if (fd_part && abs(time1) < 0.5)
        ...
        // std::cout<<"ctof : "<<data->status(part)<<std::endl;
